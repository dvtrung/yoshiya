...
class FoodsController extends Controller
{
	...
	public $listing_cols = ['id', 'name', 'price'];
	...
	{
		if(Module::hasAccess("Foods", "create")) {
	...
	{
		if(Module::hasAccess("Foods", "view")) {
	...
	{
		if(Module::hasAccess("Foods", "edit")) {			
			$food = Food::find($id);
			if(isset($food->id)) {	
				$module = Module::get('Foods');
	...
	{
		if(Module::hasAccess("Foods", "delete")) {
			Food::find($id)->delete();
	...
	{
		$values = DB::table('foods')->select($this->listing_cols)->whereNull('deleted_at');
